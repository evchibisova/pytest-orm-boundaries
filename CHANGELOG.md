...
- Aggregate definitions in `boundaries.toml` now use the extensible
  `[aggregates.<name>]` section with a required `models` list. The old
  `name = ["app.Model"]` shorthand is no longer accepted.
